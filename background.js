function startAutoClickVote(maxClicks, delayBetweenClicks, breakDuration) {
  ...
  browserApi.storage.local.get(
    ["clickCount", "totalClickCount", "captchaMissCount"],
    (data) => {
      ...
      function autoClick() {
        try {
          ...
          const radioButton = document.querySelector(
            'input[type="radio"][value="65167446"]'
          );
          const voteButton = document.getElementById("pd-vote-button14670398");
          ...
          if (radioButton && voteButton) {
            ...
            const intervalId1 = setInterval(() => {
              ...
              if (captchaElement) {
                ...
                const [operand1, operand2] = captchaText
                  .match(/\d+/g)
                  .map(Number);
                const result = operand1 + operand2;

                const answerInput = document.querySelector(
                  '.h-captcha input[type="text"]'
                );
                if (answerInput) {
                  answerInput.value = result;
                  const submitButton = document.getElementById(
                    "pd-vote-button14670398"
                  );
                  if (submitButton) {
                    ++totalClickCount;
                    ++clickCount;
                    ...
                    new Promise((resolve) => {
                        ...
                        mutations.forEach((mutation) => {
                          if (mutation.type === "childList") {
                            const captchaResponse =
                              document.querySelector(".pds-question-top");
                            if (
                              captchaResponse &&
                              captchaResponse.textContent.includes(
                      ...
                      .catch((error) => {
                        console.error(
                          "Error while waiting for captcha submission:",
                          error
                        );
                      });
                  }
